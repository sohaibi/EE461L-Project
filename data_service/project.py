...
def handle_project_creation(creation_date, project_name, user_id):
    ...
    hardware_set_dict = {}
    status = "ongoing"
    post = {"creation_date": creation_date, "project_name": project_name, "user_id": user_id,
            "status": status, "hardware_set_dict": hardware_set_dict, "history_dict": history_dict}
    print("post created")
...
def handle_set_project_name(project_id, project_name):
    collection.update_one({"_id": project_id}, {
...
def handle_update_hardware(project_id, key, data):
    ...
    collection.update_one({"_id": project_id}, {
                          "$set": {"hardware_set_dict": hardware_dict}})
